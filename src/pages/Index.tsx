      ...
      <nav className="space-y-2">
        {mainSections.map(section => (
          <button
            ...
            style={section.sectionId === activeMainSection ? { backgroundColor: PRIMARY_COLOR, color: 'white' } : {color: 'white'}} // Text color white for inactive
            ...
            onMouseOut={(e) => { if (section.sectionId !== activeMainSection) {e.currentTarget.style.backgroundColor = 'transparent'; e.currentTarget.style.color = 'white';}}} // Text color white for inactive
          >
...
`;
    try {
        ...
        if (result.candidates && result.candidates[0]?.content?.parts?.[0]?.text) { setAiAnalysisResult(result.candidates[0].content.parts[0].text); } else { setAiAnalysisResult("No response or unexpected format from AI."); }
    } catch (error) { setAiAnalysisResult(`Error: Could not retrieve AI analysis. ${error.message}`); } finally { setIsAiLoading(false); }
  };
  ...
  const selectedUnitData = useMemo(() => {
    ...
    return {
        ...unit,
        displayConsumption: displayConsumption, // Consumption value for the card based on month filter
