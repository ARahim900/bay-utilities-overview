...
const Sidebar = ({
  ...
  return (
    <div className={`${isCollapsed ? 'w-16' : 'w-64'} text-slate-100 p-5 space-y-8 min-h-screen shadow-2xl print:hidden transition-all duration-300 ease-in-out`} style={{ backgroundColor: PRIMARY_COLOR_DARK }}>
      <div className="flex items-center justify-between">
        <div className={`text-3xl font-bold flex items-center space-x-3 text-white ${isCollapsed ? 'justify-center' : ''}`}>
          ...
          {!isCollapsed && (
            <div>
              <span className="font-bold text-lg">Muscat Bay</span>
              <p className="text-sm text-gray-200">Assets & Ops</p>
            </div>
          )}
        </div>
        <button
      ...
      <nav className="space-y-2">
        {mainSections.map(section => {
          const isActive = section.sectionId === activeMainSection;
          const content = section.path ? (
            <Link
              to={section.path}
              className={`w-full flex items-center ${isCollapsed ? 'justify-center' : 'space-x-3'} p-3 rounded-lg transition-all duration-200 ease-in-out group hover:text-white`}
              style={isActive ? { backgroundColor: PRIMARY_COLOR, color: 'white' } : { color: 'white' }}
              onMouseOver={(e) => {
      ...
      {!isCollapsed && (
        <div className="mt-auto p-4 bg-slate-700 bg-opacity-30 rounded-lg text-center border" style={{ borderColor: PRIMARY_COLOR_LIGHT }}>
          <p style={{ color: PRIMARY_COLOR_LIGHT }} className="text-sm text-slate-50">Operations Management Suite</p>
          <button
      ...
      <div className="flex items-center space-x-2 sm:space-x-3 md:space-x-5">
        <div className="relative">
          ...
          <input 
            type="text" 
            placeholder="Search..." 
            className="pl-11 pr-4 py-2.5 w-full sm:w-48 md:w-72 border border-slate-300 rounded-lg focus:ring-2 outline-none text-sm transition-all focus:ring-purple-500" 
          />
        </div>
        <button className="p-2.5 rounded-lg hover:bg-slate-100 transition-colors group hidden sm:block">
          <SlidersHorizontal size={22} className="text-slate-600 group-hover:text-slate-800" />
        </button>
        <button className="p-2.5 rounded-lg hover:bg-slate-100 transition-colors group hidden md:block">
          <Share2 size={22} className="text-slate-600 group-hover:text-slate-800" />
        </button>
        <button className="p-2.5 rounded-lg hover:bg-slate-100 relative transition-colors group">
          <Bell size={22} className="text-slate-600 group-hover:text-slate-800" />
          <span className="absolute top-1 right-1 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-white"></span>
        </button>
        <div className="flex items-center space-x-3 cursor-pointer group">
          <img 
            src={`https://placehold.co/40x40/${PRIMARY_COLOR.substring(1)}/FFFFFF?text=MB&font=Inter`} 
            alt="User Avatar" 
            className="w-10 h-10 rounded-full border-2 transition-all" 
            style={{ borderColor: PRIMARY_COLOR_LIGHT }} 
            onMouseOver={(e) => e.currentTarget.style.borderColor = PRIMARY_COLOR} 
            onMouseOut={(e) => e.currentTarget.style.borderColor = PRIMARY_COLOR_LIGHT} 
          />
    ...
    <div>
      ...
      <div className="relative">
        <select 
          ...
          value={value} 
          onChange={onChange} 
          className="appearance-none w-full p-2.5 pr-10 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:outline-none bg-white text-slate-700 focus:ring-purple-500" 
        >
          {options.map(option => (
    ...
    <div className="bg-white shadow p-4 rounded-lg mb-6 print:hidden sticky top-[110px] md:top-[88px] z-10">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 items-end">
        ...
        <button 
          ...
          className="text-white py-2.5 px-4 rounded-lg text-sm font-medium transition-colors flex items-center justify-center space-x-2 h-[46px] w-full lg:w-auto" 
          style={{ backgroundColor: PRIMARY_COLOR_DARK }} 
          onMouseOver={(e) => e.currentTarget.style.backgroundColor = PRIMARY_COLOR} 
          onMouseOut={(e) => e.currentTarget.style.backgroundColor = PRIMARY_COLOR_DARK}
        >
          <Filter size={16} />
    ...
    <div className="mt-4 text-center">
      <ul className="flex flex-wrap justify-center gap-x-4 gap-y-2">
        {payload.map((entry: any, index: number) => (
          <li key={`item-${index}`} className="flex items-center text-xs text-slate-600">
            <span style={{ backgroundColor: entry.color }} className="w-3 h-3 rounded-sm mr-2 inline-block"></span>
            {entry.payload.name}: {entry.payload.value.toLocaleString()} kWh ({((entry.payload.value / totalForLegend) * 100).toFixed(1)}%)
          </li>
        ))}
      </ul>
    </div>
  ...
  return (
    <div className="flex-1 p-8 space-y-8">
      <div className="bg-white p-10 rounded-xl shadow-lg text-center">
        <h2 className="text-3xl font-bold text-slate-700 mb-4">{title}</h2>
        <p className="text-slate-500">This section is under construction. Content for {title.toLowerCase()} will be available soon.</p>
        <IconComponent size={48} className="mx-auto mt-6 text-slate-400" style={{ color: PRIMARY_COLOR_LIGHT }} />
      </div>
    </div>
  );
};
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white p-6 rounded-lg shadow-xl max-w-lg w-full max-h-[80vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-xl font-semibold" style={{ color: PRIMARY_COLOR }}>✨ AI Consumption Analysis</h3>
        ...
        {isLoading ? (
          <div className="text-center py-8">
            <Sparkles size={48} className="mx-auto animate-pulse" style={{ color: PRIMARY_COLOR_LIGHT }} />
...
`;
    try {
      ...
      setAiAnalysisResult(`Error: Could not retrieve AI analysis. ${(error as Error).message}`);
    } finally {
  ...
  const consumptionByCategoryPerformance = useMemo(() => {
    ...
    return Object.entries(categoryTotals).map(([name, value]) => ({
      name,
      consumption: parseFloat((value as number).toFixed(2))
        ...
        return (
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <SummaryCard title="Active Meters" value={activeMeters.toString()} unit="units" icon={Users2} trend="In selection" trendColor="text-slate-500 font-medium" iconBgColor="#10B981" />
            </div>
                ...
                <ChartWrapper title="Consumption Trend (All Months)" subtitle={`For category: ${selectedCategory}`}>
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={monthlyTrendForAllMonths} margin={{ top: 5, right: 30, left: 0, bottom: 5 }}>
                      ...
                      <Area type="monotone" dataKey="total" stroke={PRIMARY_COLOR} fillOpacity={1} fill="url(#colorTotal)" />
                      <Line type="monotone" dataKey="total" stroke={PRIMARY_COLOR} strokeWidth={3} activeDot={{ r: 7, strokeWidth: 2, fill: PRIMARY_COLOR }} dot={{ r: 4, fill: PRIMARY_COLOR }} name="Total kWh" />
                    </LineChart>
                  </ResponsiveContainer>
                </ChartWrapper>
              </div>
                    ...
                    <PieChart>
                      <Pie data={consumptionByTypeChartData} dataKey="value" nameKey="name" cx="50%" cy="45%" innerRadius={60} outerRadius={90} fill="#8884d8" paddingAngle={2} cornerRadius={5}>
                        {consumptionByTypeChartData.map((entry, index) => (
                          <Cell key={`cell-${index}`} fill={PIE_COLORS[index % PIE_COLORS.length]} className="focus:outline-none hover:opacity-80 transition-opacity" stroke="none" />
                        ))}
                        <Label value={`${totalForTypeChartLabel.toLocaleString(undefined, { maximumFractionDigits: 0 })}`} position="centerBottom" dy={-5} className="text-2xl font-bold fill-slate-700" />
                      ...
                      <Legend content={<DonutLegend data={consumptionByTypeChartData} />} verticalAlign="bottom" wrapperStyle={{ paddingTop: '15px' }} />
                    </PieChart>
                  </ResponsiveContainer>
                </ChartWrapper>
              </div>
            ...
            <div className="grid grid-cols-1">
              <ChartWrapper title="Top 7 Consumers" subtitle={`For ${selectedMonth}`}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={topConsumersChartData} layout="vertical" margin={{ top: 5, right: 30, left: 100, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" horizontal={false} />
                    <XAxis type="number" tick={{ fontSize: 12, fill: '#64748b' }} />
                    <YAxis dataKey="name" type="category" tick={{ fontSize: 11, fill: '#334155', width: 95 }} interval={0} width={100} />
                    <Tooltip contentStyle={{ backgroundColor: 'white', borderRadius: '8px', borderColor: '#e2e8f0' }} itemStyle={{ color: PIE_COLORS[2] }} labelStyle={{ color: '#0f172a', fontWeight: 'bold' }} />
                    <Bar dataKey="consumption" fill={PIE_COLORS[2]} name="Total kWh" barSize={18} radius={[0, 5, 5, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </ChartWrapper>
            </div>
            <div className="bg-white p-6 rounded-xl shadow-lg">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-5">
                <h3 className="text-xl font-semibold text-slate-700 mb-2 sm:mb-0">Detailed Consumption Data ({selectedMonth})</h3>
                <span className="text-sm text-slate-500">{kpiAndTableData.length} records found</span>
              </div>
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-200 text-sm">
                  <thead className="bg-slate-100">
                    <tr>
                      {['SL No.', 'Unit Name', 'Category', 'Meter No.', selectedMonth === "All Months" ? "Total kWh (All Months)" : `${selectedMonth} kWh`, ...(selectedMonth === "All Months" ? availableMonths : [])].map(header => (
                        <th key={header} scope="col" className="px-4 py-3.5 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">
                          {header}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody className="bg-white divide-y divide-slate-200">
                    {paginatedTableData.length > 0 ? paginatedTableData.map((item) => (
                      <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                        <td className="px-4 py-3 whitespace-nowrap text-slate-500">{item.slNo}</td>
                        <td className="px-4 py-3 whitespace-nowrap font-semibold" style={{ color: PRIMARY_COLOR_DARK }}>{item.unitName}</td>
                        <td className="px-4 py-3 whitespace-nowrap text-slate-600">{item.category}</td>
                        <td className="px-4 py-3 whitespace-nowrap text-slate-600">{item.meterAccountNo}</td>
                        <td className="px-4 py-3 whitespace-nowrap text-slate-800 font-bold text-right tabular-nums">
                          {item.totalConsumption.toLocaleString()}
                        </td>
                        {selectedMonth === "All Months" && availableMonths.map(month => (
                          <td key={month} className="px-4 py-3 whitespace-nowrap text-slate-700 text-right tabular-nums">{item.consumption[month].toLocaleString()}</td>
                        ))}
                      </tr>
                    )) : (
                      <tr>
                        <td colSpan={selectedMonth === "All Months" ? 5 + availableMonths.length : 5} className="text-center py-10 text-slate-500">No data matches your current filters.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
              </div>
              {kpiAndTableData.length > 0 && (
                <div className="mt-6 flex flex-col sm:flex-row justify-between items-center text-sm">
                  <span className="text-slate-600 mb-2 sm:mb-0">
                    Showing <span className="font-semibold text-slate-700">{((currentPage - 1) * itemsPerPage) + 1}</span>-
                    <span className="font-semibold text-slate-700">{Math.min(currentPage * itemsPerPage, kpiAndTableData.length)}</span> of 
                    <span className="font-semibold text-slate-700">{kpiAndTableData.length}</span> units
                  </span>
                  <div className="space-x-1.5">
                    <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="px-3.5 py-1.5 border border-slate-300 rounded-md hover:bg-slate-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-slate-700">Previous</button>
                    <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="px-3.5 py-1.5 border border-slate-300 rounded-md hover:bg-slate-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-slate-700">Next</button>
                  </div>
                </div>
              )}
            </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={consumptionByCategoryPerformance} layout="horizontal" margin={{ top: 5, right: 30, left: 20, bottom: 60 }}>
                  ...
                  <Bar dataKey="consumption" name="kWh" radius={[5, 5, 0, 0]}>
                    {consumptionByCategoryPerformance.map((entry, index) => (
